...
while [[ -f	"${fm_files[0]}" ]]; do
	...
	export BASENAME=$(basename "${fm_files[0]}"|sed 's/[^a-zA-Z0-9.]\+/_/g')
	export BACKUPFILE=$BACKUPDIR/${DATE}_${BASENAME}

	cp "${fm_files[0]}" $BACKUPFILE
	jpegoptim --strip-all --all-progressive -f $BACKUPFILE

	convert -format "jpg" -density 72x72 -quality 50 -resize 1080x "${fm_files[0]}" "$DRIVEDIR/$BASENAME"
