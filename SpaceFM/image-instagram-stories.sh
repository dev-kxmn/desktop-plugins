#!/bin/bash
#
# Cortar imagens no padrão Histórias do Instagram
#

$fm_import    # import file manager variables (scroll down for info)

while [[ -f "${fm_files[0]}" ]] ; do
	dir=$(dirname "${fm_files[0]}")
	igStoriesDir=$dir/InstagramStories
	outputfile=$(basename "${fm_files[0]}"| sed 's/[^0-9a-zA-Z.]\+/_/g;s/\.[a-z0-9A-Z]\{1,4\}//g')
	mkdir -p "$igStoriesDir";
	convert -format "jpg" -resize x1600 -gravity center -crop 900x1600+0+0 "${fm_files[0]}" "$igStoriesDir/$outputfile.jpg";
	fm_files=("${fm_files[@]:1}");
done
